...
Note: the power mode need to match the mcu selection and external power supply circuit.
        for 100-pin mcu, the power mode only supports PMU_BYPASS.
...
#else
    nvic_vector_table_set(NVIC_VECTTAB_ENVM, VECT_TAB_OFFSET);
#endif
}
...
*/
uint32_t gd32h77x_firmware_version_get(void)
{
    return (uint32_t)__GD32H77X_STDPERIPH_VERSION;
}
